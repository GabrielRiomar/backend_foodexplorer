...
class CartsService {
  ...
  async create({ dish_id, user_id, quantity }) {
    const checkCartsExists = await this.cartsRepository.show({
      user_id,
      dish_id
    })
    ...
    if (checkCartsExists) {
      if (checkCartsExists.quantity !== quantity) {
        ...
        const cartsUpdated = await this.cartsRepository.update({
          id,
          user_id,
          quantity
        })
      ...
      throw new AppError('Item already added on cart', 401)
    }

    const cartsCreated = this.cartsRepository.create({
      dish_id,
      user_id,
      quantity
    })
